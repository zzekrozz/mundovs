...
function EmblemInfinite() {
  return (
    <svg width="64" height="64" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      {/* Outer ring */}
      <circle cx="32" cy="32" r="28" fill="none" stroke="#1a4f63" strokeWidth="1.2" opacity=".4"/>
      {/* Infinity symbol */}
      <path d="M 16 32 C 16 24, 24 24, 28 32 C 32 40, 40 40, 44 32 C 48 24, 40 24, 36 32 C 32 40, 24 40, 20 32 C 16 24, 16 24, 16 32 Z"
            fill="none" stroke="#1d9e75" strokeWidth="1.8" strokeLinecap="round"/>
      {/* Arrow up */}
      <polygon points="14,18 11,22 17,22" fill="#1d9e75" opacity=".85"/>
      {/* Arrow down */}
      <polygon points="50,46 47,42 53,42" fill="#1a6080" opacity=".85"/>
    </svg>
  );
}
...
function InfiniteRecord() {
  ...
  useEffect(() => {
    try {
      const v = parseInt(localStorage.getItem("mundovs_higherlower_best_global") || "0", 10);
      setBest(v || 0);
    } catch (e) {}
  }, []);
  if (!best) return null;
  return <div className="mv-card-stat">RÉCORD: {best}</div>;
}
  ...
  return (
    <Layout>
      ...
      <div className="mv-modes">
        ...
        {/* Higher / Lower VS — card grande */}
        <Link href="/infinito" className="mv-card mv-card-featured mv-card-infinite">
          <div className="mv-card-topline mv-topline-infinite" />
          <div className="mv-card-inner">
            <div className="mv-card-emblem">
              <EmblemInfinite />
            </div>
            <div className="mv-card-body">
              ...
              <h2 className="mv-card-title">Higher / Lower VS</h2>
              <p className="mv-card-desc">
                Encadena comparaciones: ¿tiene más o menos que el siguiente país?
              </p>
              <InfiniteRecord />
              <div className="mv-card-cta">Jugar infinito ›</div>
            </div>
          </div>
        </Link>
      </div>
