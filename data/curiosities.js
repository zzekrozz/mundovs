...
function fallback(cA, cB) {
  const contA = cA.continent || cA.region || "su región";
  const contB = cB.continent || cB.region || "su región";
  if (contA === contB && contA !== "su región") {
    return `${cA.name} y ${cB.name} están ambos en ${contA}, pero tienen realidades muy distintas.`;
  }
  return `${cA.name} y ${cB.name} muestran cómo el mundo es enormemente diverso.`;
}
