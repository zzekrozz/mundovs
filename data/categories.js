...
export const CATEGORIES = [
  ...
  // ========== NUEVAS CATEGORÍAS PILOTO ==========
  {
    ...
    phrase: (a, b) => {
      ...
      if (diff < 1) return "casi la misma esperanza de vida";
      return diff.toFixed(1) + " años más de esperanza de vida";
    },
    dir: "higher",
  },
  {
    key: "corruption_cpi", label: "Corrupción (CPI)", group: "sociedad",
    question: "¿Dónde hay menos corrupción?",
    gradient: "linear-gradient(135deg, #C5CAE9 0%, #9FA8DA 100%)",
    backgrounds: ["/illustrations/education.svg"],
    format: (v) => v + " pts (0-100)",
    phrase: (a, b) => {
      const diff = Math.abs(a - b);
      if (diff < 5) return "muy parecido nivel de percepción de corrupción";
      if (a > b) return diff.toFixed(0) + " puntos menos de corrupción";
      return "tiene mejor índice de corrupción";
    },
    dir: "higher", // Mayor puntuación = menor corrupción
  },
  {
    key: "rainfall_mm", label: "Precipitación", group: "geografia",
    question: "¿Dónde llueve más?",
    gradient: "linear-gradient(135deg, #B3E5FC 0%, #81D4FA 100%)",
    backgrounds: ["/illustrations/forest.svg"],
    format: (v) => v.toLocaleString() + " mm/año",
    phrase: (a, b) => {
      const r = Math.max(a,b) / Math.min(a,b);
      const diff = Math.abs(a - b);
      if (r >= 2) return r.toFixed(1) + "x más lluvia";
      return diff.toLocaleString() + " mm más al año";
    },
    dir: "higher",
  },
  {
    key: "forest_area_percent", label: "Cobertura forestal", group: "geografia",
    question: "¿Qué país tiene más bosque proporcionalmente?",
    gradient: "linear-gradient(135deg, #C8E6C9 0%, #A5D6A7 100%)",
    backgrounds: ["/illustrations/forest.svg"],
    format: (v) => v.toFixed(1) + "% territorio",
    phrase: (a, b) => {
      const diff = Math.abs(a - b);
      if (diff < 1) return "casi la misma cobertura forestal";
      return diff.toFixed(1) + "% más de bosques";
    },
    dir: "higher",
  },
  // ========== FASE 2 (6 CATEGORÍAS) ==========
  {
    key: "land_borders_count", label: "Fronteras terrestres", group: "geografia",
    question: "¿Qué país limita con más países?",
    gradient: "linear-gradient(135deg, #FFCCBC 0%, #FFAB91 100%)",
    backgrounds: ["/illustrations/map.svg"],
    format: (v) => v + " países fronterizos",
    phrase: (a, b) => {
      const diff = Math.abs(a - b);
      if (diff === 0) return "limitan con el mismo número de países";
      if (diff === 1) return "limita con 1 país más";
      return "limita con " + diff + " países más";
    },
    dir: "higher",
  },
  {
    key: "capital_elevation_m", label: "Capital más alta", group: "geografia",
    question: "¿Qué país tiene la capital a más altura?",
    gradient: "linear-gradient(135deg, #E1F5FE 0%, #B3E5FC 100%)",
    backgrounds: ["/illustrations/mountain.svg"],
    format: (v) => v.toLocaleString() + " m",
    phrase: (a, b) => {
      const diff = Math.abs(a - b);
      if (diff < 100) return "tienen capitales a similar altura";
      return "su capital está " + Math.round(diff) + "m más alta";
    },
    dir: "higher",
  },
  {
    key: "skyscrapers_150m", label: "Rascacielos (150m+)", group: "poder",
    question: "¿Qué país tiene más rascacielos?",
    gradient: "linear-gradient(135deg, #F3E5F5 0%, #E1BEE7 100%)",
    backgrounds: ["/illustrations/city.svg"],
    format: (v) => v.toLocaleString() + " edificios",
    phrase: (a, b) => {
      const r = Math.max(a,b) / Math.min(a,b);
      if (Math.min(a,b) === 0) return "uno tiene rascacielos, el otro no";
      if (r >= 2) return r.toFixed(1) + "x más rascacielos";
      return Math.abs(a-b) + " rascacielos más";
    },
    dir: "higher",
  },
  {
    key: "billionaires_count", label: "Multimillonarios", group: "poder",
    question: "¿Qué país tiene más multimillonarios?",
    gradient: "linear-gradient(135deg, #FFF9C4 0%, #FFF59D 100%)",
    backgrounds: ["/illustrations/wealth.svg"],
    format: (v) => v.toLocaleString() + " billonarios",
    phrase: (a, b) => {
      const r = Math.max(a,b) / Math.min(a,b);
      if (Math.min(a,b) === 0) return "uno tiene billonarios, el otro no";
      if (r >= 2) return r.toFixed(0) + "x más multimillonarios";
      return Math.abs(a-b) + " billonarios más";
    },
    dir: "higher",
  },
  {
    key: "meat_consumption_kg", label: "Consumo de carne", group: "cultura",
    question: "¿Dónde se come más carne?",
    gradient: "linear-gradient(135deg, #FFCCBC 0%, #FFAB91 100%)",
    backgrounds: ["/illustrations/food.svg"],
    format: (v) => v.toFixed(1) + " kg/persona/año",
    phrase: (a, b) => {
      const diff = Math.abs(a - b);
      const ratio = Math.max(a,b) / Math.min(a,b);
      if (ratio >= 1.5) return (ratio.toFixed(1)) + "x más consumo de carne";
      return diff.toFixed(1) + " kg/año más";
    },
    dir: "higher",
  },
  {
    key: "homicide_rate_per_100k", label: "Tasa de homicidios", group: "sociedad",
    question: "¿Dónde es más seguro vivir?",
    gradient: "linear-gradient(135deg, #EF9A9A 0%, #EF5350 100%)",
    backgrounds: ["/illustrations/safety.svg"],
    format: (v) => v.toFixed(1) + " por 100k/año",
    phrase: (a, b) => {
      const diff = Math.abs(a - b);
      if (diff < 1) return "tienen tasas de seguridad similares";
      // IMPORTANTE: Mayor tasa = MENOS seguro, así que invierte el lenguaje
      const safest = a < b ? "A" : "B";
      return "es " + diff.toFixed(1) + " veces más seguro";
    },
    dir: "lower", // ⚠️ IMPORTANTE: GANA EL VALOR MENOR
  },
];
