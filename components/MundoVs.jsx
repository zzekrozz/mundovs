...
function Summary({ cA, cB, userScore, totalSteps, countryWins, stepResults, onNewGame, onReplay }) {
  ...
  // Causa común de "pantalla en blanco": stepResults vacío o con un
  // elemento sin step. Si pasa, mostramos un fallback útil en vez de
  // que React lance error.
  ...
  if (!cA || !cB) {
    return (
      <div className="game-wrap mv-game-dark">
        ...
        <div className="summary-card" style={{ textAlign: "center", padding: "2rem 1rem" }}>
          ...
          <button className="btn-next" onClick={onNewGame}>Volver al menú</button>
        </div>
      </div>
    );
  }
  ...
  const urClass = safeScore >= 4 ? "ur-good" : safeScore >= 2 ? "ur-ok" : "ur-bad";
  const urMsg = safeScore >= 4
    ? `🧠 ¡Experto en geografía! ${safeScore}/${safeTotal}`
    : safeScore >= 2
      ? `Nada mal — ${safeScore}/${safeTotal} acertadas`
      : `A estudiar geografía… ${safeScore}/${safeTotal}`;
    ...
    <div className="game-wrap mv-game-dark">
      ...
      <div className="summary-card">
        ...
        <div className="big-score">
          <div className={`big-cell${overall === "A" ? " winner" : ""}`}>
          ...
          <div className={`big-cell${overall === "B" ? " winner" : ""}`}>
        ...
        {winC ? (
          <div style={{ textAlign: "center", fontSize: 14, fontWeight: 500, marginBottom: "0.75rem", color: "var(--mv-text)" }}>
            {winC.flag} {winC.name} gana la comparación
          </div>
        ) : (
          <div style={{ textAlign: "center", fontSize: 14, fontWeight: 500, marginBottom: "0.75rem", color: "var(--mv-text)" }}>
            ¡Empate entre los dos países!
          </div>
        )}
        {safeResults.length > 0 ? (
          <div className="recap">
            {safeResults.map((r, i) => {
              const wname = r.winner === "A" ? `${cA.flag} ${cA.name}` :
                            r.winner === "B" ? `${cB.flag} ${cB.name}` : "Empate";
              const label = r.step && r.step.label ? r.step.label : `Ronda ${i + 1}`;
              return (
                <div key={i} className="recap-row">
                  <span className="recap-cat">{label}</span>
                  <div className="recap-right">
                    <div className="dot-s" style={{ background: r.correct ? "var(--mv-green)" : "var(--mv-red)" }} />
                    <span className="recap-win">{wname}</span>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ textAlign: "center", fontSize: 12, color: "var(--mv-text-dim)", padding: "0.5rem 0" }}>
            No hay desglose disponible para esta partida.
          </div>
        )}
      </div>
