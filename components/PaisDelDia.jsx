...
export default function PaisDelDia({ dayOffset = 0 }) {
  ...
  useEffect(() => {
    ...
    // Para días anteriores, usar un storage key diferente
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(storageKey, JSON.stringify({ date: todayStr, dayNumber, revealedHints, attempts, gameStatus }));
      
      // Solo actualizar racha si es el día actual (no archivo)
  ...
  // Calcular fecha real basada en dayNumber
  ...
  return (
    <div className="pdd-container">
      ...
      <div className="pdd-header">
        <div className="pdd-day-badge">{isArchive ? "ARCHIVO · " : ""}DÍA #{dayNumber}</div>
        ...
        {!isArchive && (
          <a href="/pais-del-dia/archivo" className="pdd-archive-link">
            📅 Ver retos anteriores
          </a>
        )}
        {isArchive && (
          <a href="/pais-del-dia" className="pdd-archive-link">
            ← Volver al reto de hoy
          </a>
        )}
      </div>
